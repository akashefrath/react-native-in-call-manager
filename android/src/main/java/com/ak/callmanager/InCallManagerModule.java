...
public class InCallManagerModule extends ReactContextBaseJavaModule implements LifecycleEventListener {
    ...
    /*
     * // --- TODO: AudioDeviceCallBack android sdk 23+ if
     * (android.os.Build.VERSION.SDK_INT >= 23) { private class
     * MyAudioDeviceCallback extends AudioDeviceCallback { public void
     * onAudioDevicesAdded(AudioDeviceInfo[] addedDevices) { mAddCallbackCalled =
     * true; } public void onAudioDevicesRemoved(AudioDeviceInfo[] removedDevices) {
     ...
     * public void test_deviceCallback() { AudioDeviceCallback callback = new
     * EmptyDeviceCallback(); mAudioManager.registerAudioDeviceCallback(callback,
     * null); }
     * 
     * // --- get all audio devices by flags //public AudioDeviceInfo[] getDevices
     * (int flags) //Returns an array of AudioDeviceInfo objects corresponding to
     * the audio devices currently connected to the system and meeting the criteria
     * specified in the flags parameter. //flags int: A set of bitflags specifying
     * the criteria to test. }
     * 
     * // --- TODO: adjust valume if needed. if (android.os.Build.VERSION.SDK_INT >=
     * 21) { isVolumeFixed ()
     * 
     * // The following APIs have no effect when volume is fixed: adjustVolume(int,
     * int) adjustSuggestedStreamVolume(int, int, int) adjustStreamVolume(int, int,
     * int) setStreamVolume(int, int, int) setRingerMode(int) setStreamSolo(int,
     * boolean) setStreamMute(int, boolean) }
     * 
    ...
    private void requestAudioFocus() {
        if (!isAudioFocused) {
            int result = audioManager.requestAudioFocus(mOnFocusChangeListener, AudioManager.STREAM_VOICE_CALL,
    ...
    /**
     * This is part of start() process. ringbackUriType must not empty. empty means
     * do not play.
     */
    @ReactMethod
    public void startRingback(final String ringbackUriType) {
        ...
        try {
            ...
            /*
             * TODO: for API 21 data.put("audioFlag",
             * AudioAttributes.FLAG_AUDIBILITY_ENFORCED); data.put("audioUsage",
             * AudioAttributes.USAGE_VOICE_COMMUNICATION); //
             * USAGE_VOICE_COMMUNICATION_SIGNALLING ? data.put("audioContentType",
             * AudioAttributes.CONTENT_TYPE_SPEECH); // CONTENT_TYPE_MUSIC ?
             */
    ...
    /**
     * This is part of start() process. busytoneUriType must not empty. empty means
     * do not play. return false to indicate play tone failed and should be stop()
     * immediately otherwise, it will stop() after a tone completed.
     */
    public boolean startBusytone(final String busytoneUriType) {
        ...
        try {
             ...
             * USAGE_VOICE_COMMUNICATION ? data.put("audioContentType",
    ...
    public void startRingtone(final String ringtoneUriType, final int seconds) {
        try {
            ...
            /*
             * TODO: for API 21 data.put("audioFlag", 0); data.put("audioUsage",
             * AudioAttributes.USAGE_NOTIFICATION_RINGTONE); //
             * USAGE_NOTIFICATION_COMMUNICATION_REQUEST ? data.put("audioContentType",
    ...
    private class myMediaPlayer extends MediaPlayer implements MyPlayerInterface {
        ...
        public void startPlay(final Map data) {
            try {
                ...
                /*
                 * // TODO: use modern and more explicit audio stream api if
                 * (android.os.Build.VERSION.SDK_INT >= 21) { int audioFlag = (Integer)
                 * data.get("audioFlag"); int audioUsage = (Integer) data.get("audioUsage"); int
                 * audioContentType = (Integer) data.get("audioContentType");
                 * 
                 * setAudioAttributes( new AudioAttributes.Builder() .setFlags(audioFlag)
                 * .setLegacyStreamType(stream) .setUsage(audioUsage)
                 * .setContentType(audioContentType) .build() ); }
                 */
    ...
    private String _checkPermission(String targetPermission) {
        try {
            ...
            if (ContextCompat.checkSelfPermission(reactContext,
                    targetPermission) == PackageManager.PERMISSION_GRANTED) {
                return "granted";
            } else {
                return "denied";
            }
    ...
    @ReactMethod
    public void GetAudioDevices(Callback cb) {
        try {
            updateAudioDeviceState();
            cb.invoke(null, audioDevices.toString());
            Log.e(TAG, "selectAudioDevice() Can not select " + "    " + " from available " + audioDevices);
    ...
    private void _requestPermission(String targetPermission, Promise promise) {
         ...
         * showMessageOKCancel("You need to allow access to microphone for making call",
         * new DialogInterface.OnClickListener() {
         * 
         * @Override public void onClick(DialogInterface dialog, int which) {
         * ActivityCompat.requestPermissions(currentActivity, new String[]
         * {permission.RECORD_AUDIO}, requestPermissionCode); } }); return; }
         */
        ...
        if (promise != null && targetPermission != null) {
            ...
            if (!permissionResultMap.containsKey(targetPermission)) {
                Log.wtf(TAG, String.format(
                        "RNInCallManager.onRequestPermissionsResult(): requested permission %s but did not appear",
                        targetPermission));
                promise.reject(String.format("%s_PERMISSION_NOT_FOUND", targetPermission),
                        String.format("requested permission %s but did not appear", targetPermission));
                return;
            }
    ...
    public Set <AudioDevice> getdevices (){
        Log.e(TAG, String.valueOf(audioDevices));
        return  audioDevices;
    ...
    /*
     * private void setSpeakerphoneOn(boolean on) { boolean wasOn =
     * audioManager.isSpeakerphoneOn(); if (wasOn == on) { return; }
     * audioManager.setSpeakerphoneOn(on); }
     */

    /** Sets the microphone mute state. */
    /*
     * private void setMicrophoneMute(boolean on) { boolean wasMuted =
     * audioManager.isMicrophoneMute(); if (wasMuted == on) { return; }
    ...
    public void updateAudioDeviceState() {
        ...
        if (selectedAudioDevice == AudioDevice.BLUETOOTH && newAudioDevice != AudioDevice.BLUETOOTH
        ...
        if (newAudioDevice != selectedAudioDevice || audioDeviceSetUpdated) {
            ...
            /*
             * if (audioManagerEvents != null) { // Notify a listening client that audio
